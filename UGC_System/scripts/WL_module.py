...
class RegularBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # print("checccck")
        mult = torch.matmul(mlp1, mlp2.T)

        # print(inputs.shape, mult.shape)

        out = mult#self.skip(in1=inputs, in2=mult)
        return out
...
class MlpBlock(nn.Module):
    ...
    def __init__(self, in_features, out_features, depth_of_mlp, activation_fn=nn.functional.relu):
        ...
        for i in range(depth_of_mlp):
            self.convs.append(nn.Linear(in_features, out_features))#, kernel_size=1, padding=0, bias=True))
...
class SkipConnection(nn.Module):
    ...
    def __init__(self, in_features, out_features):
        super().__init__()
        self.conv = nn.Linear(in_features, out_features)#, kernel_size=1, padding=0, bias=True)
