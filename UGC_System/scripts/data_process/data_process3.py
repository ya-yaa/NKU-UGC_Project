# 按位置对齐匹配
# python f:\毕设\Social-Network\data_process3.py --dataset facebook --out-subdir ugc_v2
...
# --- 核心改进：并查集算法实现 V3 标签策略 ---
def _load_labels_txt_v3(path: Path, num_nodes: int) -> tuple[np.ndarray, int, int]:
    """
    V3 策略：位置对齐匹配。第一个标签重合为一类，第二个标签重合也为一类，但不同位置不算。
    """
    ...
    with path.open("r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            for i, l in enumerate(labels):
                pos_label_pair = (i, l)
                if pos_label_pair in pos_label_to_node:
                    union(u, pos_label_to_node[pos_label_pair])
                else:
                    pos_label_to_node[pos_label_pair] = u
...
def process_one(
    ...
    # 计算 V3 标签 (并查集处理全量节点后切片)
    if labels_path.exists():
        max_orig_id = max(all_sampled_nodes)
        temp_y, num_classes, labeled = _load_labels_txt_v3(labels_path, max_orig_id + 1)
    ...
    print(f"--- Processed {dataset} using V3 Strategy (Position-wise Match) ---")
...
def main() -> None:
    ...
    p.add_argument("--out-subdir", default="ugc_v3") # 默认输出到 ugc_v3
